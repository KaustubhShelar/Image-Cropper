...
const Post = ({shape,selectedImage,setSelectedImage,setOpenPost,openPost,fileN,style,small}) => {

  const[scaleValue , setScaleValue]=useState(1);
  const[rotate , setRotate]=useState(0);

  if(small==="yes"){
    Post.height = shape==="square"?200:157;
    Post.width = shape==="square"?200:220;
    Post.border= shape==="square"?150:0;
  }else{
    Post.height = shape==="square"?300:250;
    Post.width = shape==="square"?300:350;
    Post.border= shape==="square"?150:0;
  ...
  const extractImageFileExtensionFromBase64 = (base64Data) => {
    ...
    console.log("finalFileName = "+Post.finalFileName)
    if(!Post.finalFileName){
      Post.finalFileName=fileN;
    }
    const fileName=Post.finalFileName + '.' + fileExt  ;
  ...
  const profilePicChange = (fileChangeEvent) => {
    ...
    const { name } = file
    Post.finalFileName = name.split('.').slice(0, -1).join('.')
    const { type } = file;
  ...
  return (
    ...
    maxWidth = {'md'}
    fullWidth={true}
    open={openPost} onClose={()=>setOpenPost(false)}>
     <DialogTitle align="right">
          <IconButton onClick={()=>setOpenPost(false)}>
              <CloseIcon />
            </IconButton>
        </DialogTitle>
        <div className="App">
    <label class="custom-file-upload">
      ...
      Select File
       ...
       <AvatarEditor
            ...
            height={Post.height} width={Post.width} borderRadius={Post.border}
